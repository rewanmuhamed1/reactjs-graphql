...
const Navbar = () => {
    ...
    const [toggleBurgerBtn, setToggleBurgerBtn] = useState(0);
    return (
        <nav className="bg-slate-100">
            <div className="mx-auto max-w-7xl px-2 sm:px-6 lg:px-8">
                <div className="relative flex h-16 items-center justify-between">
                    <div className="absolute inset-y-0 left-0 flex items-center sm:hidden">
                        <button type="button" onClick={() => { setToggleBurgerBtn(!toggleBurgerBtn) }} className="relative inline-flex items-center justify-center rounded-md p-2 text-gray-400 hover:bg-gray-700 hover:text-white focus:outline-none focus:ring-2 focus:ring-inset focus:ring-white" aria-controls="mobile-menu" aria-expanded="false">
                            ...
                            <div className="flex space-x-4">
                                {categories.map((category) => (
                                    <NavLink
                                        ...
                                        className="px-3 py-2 font-medium uppercase raleway hover:border-b-2 hover:border-green-500 "
                                        key={category.name}
                    ...
                    <div className="absolute inset-y-0 right-0 flex items-center pr-2 sm:static sm:inset-auto sm:ml-6 sm:pr-0">
                        <button onClick={() => { setToggleCart(!toggleCart) }} data-testid='cart-btn' className="relative inline-flex items-center">
                            ...
                            {cartItemsCount > 0 && (
                                <span className="absolute cartCount inline-flex items-center justify-center px-2 py-1 text-xs font-bold leading-none text-red-100 bg-black rounded-full">
                                    {cartItemsCount}
                                </span>
                            )}
                        </button>
            ...
            {
                toggleBurgerBtn ? <div className="sm:hidden" id="mobile-menu">
                    <div className="flex flex-col  space-y-1 px-2 pb-3 pt-2">
                        {categories.map((category) => (
                             ...
                             className="px-3 py-2 font-medium uppercase raleway hover:border-b-2 hover:border-green-500 w-24"
                             key={category.name}
                             to={`/category/${category.name}`}
